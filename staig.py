...
class STAIG:
    ...
    def train(self):
        ...
        if self.single is False:
            ...
            pseudo_labels = generate_pseudo_labels(self.adata.obsm['img_emb'], 30).float().to(self.device)
    ...
    def draw_spatial(self):
        sc.pl.spatial(self.adata,
                      img_key='hires',
                      size=1.6,
                      color=["ground_truth", "domain"], frameon=False, title=None, legend_loc=None,
                      show=True, save='spatial.png')

    def draw_single_spatial(self):
        sc.pl.embedding(self.adata, basis="spatial", color="domain",
                        save='single_spatial.png')
